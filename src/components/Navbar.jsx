...
//       {/* Gradient Glass-Like Menu Button (glitch fix) */}
...
const Navbar = () => {
  ...
  return (
    <motion.div
      ...
      animate={isOpen ? "open" : "closed"}
      initial="closed"
      className="fixed top-4 left-1/2 transform -translate-x-1/2 h-16 bg-black/60 backdrop-blur-md rounded-full shadow-lg flex items-center justify-between px-6 z-50 overflow-hidden"
    >
      {/* Logo */}
      <div className="shrink-0 flex items-center gap-2">
        <img src="/assets/JOWIS.png" alt="Logo" className="h-8 w-auto" />
      ...
      {/* Nav Links (center area grows/shrinks) */}
      ...
      {/* Menu Button (isolated so no glitch on close) */}
      <motion.div layout="position" className="shrink-0">
        <button
          onClick={() => setIsOpen(!isOpen)}
          className={`ml-auto px-4 py-2 rounded-md transition-all duration-300 shadow-md
